...
while (fontSize > 30) {
  lines = wrapText(playlistTitle, fontSize);
  const tooWide = lines.some(line => ctx.measureText(line).width > maxTextWidth);
  if (!tooWide && lines.length <= 3) break; // Limit to 3 lines max
  fontSize -= 2;
}

// If more than 3 lines, truncate last line with ellipsis
if (lines.length > 3) {
...
lines.forEach((line, i) => {
  ...
  ctx.fillText(randomTagline, centerX, canvas.height - 120);
